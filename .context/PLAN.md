...
3. **Review.** A mark sheet for the finished attempt. The stored score is the figure at the top, with correct, wrong, blank, and time used against the clock that was set. Then a section table, one row per topic. A question with no topic is grouped under "No topic". Each section shows how many questions, how many correct, wrong, and blank, the section score, and time spent. Below that, one question list. Filters narrow it: a section (or every section) and a result (everything, correct, wrong, or blank). The two filters combine. Clicking a section name, or its correct, wrong, or blank count, sets those filters. Each listed question shows its paper number, stem, your answer, the key, time, and explanation. "Export report" downloads JSON. "Save a copy" downloads one HTML file of the full mark sheet, unfiltered: wrong answers, blank answers, then every question. The attempt already holds the snapshot, so opening the attempt from history shows this report again. Export does not write a second copy into IndexedDB. A stem that puts a blank line before its code shows the sentence as text and the code as a code block.
...
- 2026-09-23 — Review filters the question list by section and by everything, correct, wrong, or blank. A blank line before code keeps the sentence out of the code block.
